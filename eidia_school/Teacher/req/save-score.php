...
if (isset($_POST['score-1']) &&
    ...
    if(empty($score_1) || empty($score_2) || empty($score_3) || empty($score_4) || empty($score_5) || empty($aoutof_1) || empty($aoutof_2) || empty($aoutof_3) || empty($aoutof_4) || empty($aoutof_5) || empty($student_id) || empty($subject_id) || empty($current_year) || empty($current_semester)){
       ...
       $em  = "All fields are required";
        ...
        $data = '';
        $limit = 0;/*$limit variable holds the sum of the maximum values. */
        /*checking each score and its corresponding maximum value (aoutof) to ensure that they meet certain conditions before appending them to the variable $data. */
        if ($score_1 <= 100 && $aoutof_1 <=100 && $score_1 > 0 && $aoutof_1 > 0 && $score_1 <=  $aoutof_1)  {
            $data .= $score_1." ".$aoutof_1; /* appends that score and its maximum value to the $data string */
             $limit += $aoutof_1;
        } 
        ...
        exit;
          ...
          /*vérifie si le formulaire contient un champ student_score_id. Cela indique si le score de l'étudiant existe déjà dans la base de données. */
        if (isset($_POST['student_score_id'])) {
          /*si il  existe, cela signifie que le score de l'étudiant doit être mis à jour */
        ...
        exit;
            ...
            /*Si student_score_id n'existe pas, cela signifie que le score de l'étudiant doit être créé pour la première fois. */
