<?php 
session_start();
//Checks if teacher_id and role are set in the session.
if (isset($_SESSION['teacher_id']) && 
    isset($_SESSION['role'])) {
    //Verifies if the user has the role of a teacher.
    // Includes necessary PHP files containing functions and data structures.
    if ($_SESSION['role'] == 'Teacher') {
       ...
       include "data/student_score.php";
        //Redirects to students.php if student_id is not set in the URL.
       ...
       // Get subjects based on the grade of the student
       $subjects = getSubjectByGrade($student['grade'], $conn);

       $teacher_id = $_SESSION['teacher_id'];
       $teacher = getTeacherById($teacher_id, $conn);
       // Extract subject IDs that the teacher is responsible for
       $teacher_subjects = str_split(trim($teacher['subjects']));

       
      // Initialize variable to store subject ID for student score retrieval
       $ssubject_id = 0;
        ...
        <?php 
           ...
           <?php } } if($counter <  5){ 
            ...
            <div class="input-group mb-3">
                ...
                <!--put the score in ther places if is empty it will show xx-->
                  <input type="text" class="form-control" value="xx" 
                  name="score-<?php echo $i; ?>">
                <!--used to sperate the score from it maximum-->
                  <span class="input-group-text">/</span>
                  <input type="text" class="form-control" value="xx"
           ...
           <input type="text" name="student_id" value="<?=$student_id?>" hidden>
            <input type="text" name="subject_id" value="<?=$ssubject_id?>"hidden>
                <!--the semester and the year taken from the setting -->
